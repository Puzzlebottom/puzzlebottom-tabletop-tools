...
├── backend/           Build & test config for Lambda code; test helpers in test/
├── modules/           Domain modules (PlayTable, DiceRoller) – resolvers, handlers, steps
├── infrastructure/    CDK stacks (includes composed GraphQL schema)
...
**Module layout** (each module under `modules/<name>/`):

```
modules/<module-name>/
├── graphql/           schema.graphql; events.graphql (optional)
├── resolvers/         AppSync resolvers
├── handlers/          EventBridge/SQS handlers (optional)
├── steps/             Step Function steps (optional)
├── shared/            Module-internal shared code (optional)
└── *.test.ts          Colocated tests
```

## GraphQL Schema & Types

The GraphQL schema is composed from module schemas (`modules/play-table/graphql/`, `modules/dice-roller/graphql/`) into `infrastructure/lib/graphql/schema.graphql` and is the single source of truth for domain types. Codegen produces both TypeScript types and Zod validation schemas in `shared/graphql-types/src/generated.ts`. Frontend and backend import from `@puzzlebottom-tabletop-tools/graphql-types` and `@puzzlebottom-tabletop-tools/schemas`.

**When you change the schema:**

1. Run `npm run codegen` to compose schemas and regenerate types
...
**EventBridge event schemas (generated from `infrastructure/lib/graphql/events.graphql`):** `RollCompletedDetailSchema`, `RollRequestCompletedDetailSchema`, `PlayerLeftDetailSchema`, `PlayerJoinedDetailSchema`, `EventDetailSchema`. The envelope schema `EventBridgeEventBodySchema` and `parseEventDetail` are hand-written. Parsed events use `__typename` for discrimination (flat structure).
...
| **steps**          | `modules/dice-roller/steps`    | domain, shared           |
| **resolvers**      | `modules/*/resolvers`          | domain, contracts        |
| **handlers**       | `modules/dice-roller/handlers` | domain, handlers, shared |
