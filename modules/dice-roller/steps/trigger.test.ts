...
import { createSqsEvent } from '../../../backend/test/sqs-event.js'
import { handler } from './trigger.js'
...
describe('trigger handler (dispatcher)', () => {
  ...
  it('invokes roll-completed handler for initiative RollCompleted', async () => {
    ...
    expect(JSON.parse(invokeParams?.Payload as string)).toMatchObject(detail)
  })
