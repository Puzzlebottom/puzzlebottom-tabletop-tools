    ...
    try {
      switch (detailType) {
        ...
        case DETAIL_TYPE_PLAYER_LEFT: {
          ...
          if (parsedEvent.__typename !== 'PlayerLeftDetail') continue
          await lambdaClient.send(
            new InvokeCommand({
          ...
          console.log(
            `Invoked player-left handler for player ${parsedEvent.id}`
          )
          break
        }
        ...
        case DETAIL_TYPE_PLAYER_JOINED: {
          const parsedEvent = parseEventDetail(envelope)
          if (parsedEvent.__typename !== 'PlayerJoinedDetail') continue
          await lambdaClient.send(
            new InvokeCommand({
          ...
          console.log(
            `Invoked player-joined handler for player ${parsedEvent.id}`
          )
          break
        }
